...
async function exampleMatchingPromptUsage() {
  ...
  console.log(userPreferences); // 넘겨줘야할 데이터 summaryPrompt => summarized 가 나오고, userPreferences 이걸 주면 createMatchingprompt로 matching prompt가 나오고 =?
  ...
  const response = await fetch(url, {
    method: "POST",
    body: JSON.stringify({
      summaryPrompt : summaryPrompt, userPreferences : userPreferences
    }),
    headers: {
