...
function createMatchingPrompt(data, userPrefer) {
  ...
  prompt += `### 사용자 선호도:\n`;
  prompt += `사용자의 성향은 액티비티를 좋아하고\n`; //${userPrefer.profilecharacter}
  ...
  prompt += `- **선호 연령대, 성별, 예산 범위는** 사용자의 선호 사항이지만, **필수 조건은 아닙니다.**  선호 조건에 얼마나 부합하는지에 따라 추천 점수(최고 점수 100점)를 부여하여\n`;
  prompt += `- 추천 점수에 따라 전부 정렬해 알려주세요..\n`; //일정 수치를 넘어가면 
...
async function exampleMatchingPromptUsage() {
  ...
  // ✅ 예산 추가
  let gender = genderFilter === 1 ? "남성" : (genderFilter === 2 ? "여성" : "알 수 없음");
  const filters = {
  ...
  const userPreferences = {
    preferredDestination: filters.destinationFilter,
    preferredAgeRange: { min: filters.minAgeFilter, max : filters.maxAgeFilter },
    preferredGender: filters.genderFilter,
